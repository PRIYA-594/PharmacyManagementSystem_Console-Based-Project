...
public class Supplier {
	...
	{
		ArrayList<Supplier> sup = SupplierDB.supplierDetailsForOrderDB(sup_id);
	...
	{
		ArrayList<Supplier> sup = SupplierDB.supplierDetailsDB();
		Supplier.displayFormat();
		for(Supplier sp:sup)
		{
			sp.display();
	...
	void addDetails()throws Exception
	{
		...
		setMail(s.next());
		SupplierDB.insertSupplierDB(getName(), getPh(), getMail());
	...
	void updateDetails()throws Exception
	{
		...
		if(validSupId(getSupId()))
		{
			...
			do
			{
				...
				SupplierDB.updateSupplierDB(field, change, getSupId());
	...
	{
		return SupplierDB.validSupIdDB(id);
	}
}
